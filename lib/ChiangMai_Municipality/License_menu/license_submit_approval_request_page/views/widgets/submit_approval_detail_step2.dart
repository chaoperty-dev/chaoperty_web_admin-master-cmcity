// ============================================================================
// submit_approval_detail_step2.dart
// ============================================================================
// Step 2 — Timeline ลำดับขั้นตอนการอนุมัติ (current_round.steps)
// UI pattern พอร์ตมาจาก FactCheck step2 (timeline vertical)
...
class SubmitApprovalDetailStep2 extends StatefulWidget {
  final String? requestUuid;
  const SubmitApprovalDetailStep2({super.key, this.requestUuid});

  @override
  State<SubmitApprovalDetailStep2> createState() =>
      _SubmitApprovalDetailStep2State();
}

class _SubmitApprovalDetailStep2State
    extends State<SubmitApprovalDetailStep2> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.requestUuid != null && widget.requestUuid!.isNotEmpty) {
        context
            ...
            .loadApprovalDetail(requestUuid: widget.requestUuid!);
      }
    });
  }

  @override
  void didUpdateWidget(covariant SubmitApprovalDetailStep2 old) {
  ...
  Widget _errorBlock(LicenseSubmitApprovalDetailViewModel vm) {
    if (vm.detailError == null) return const SizedBox.shrink();
    return Padding(
...
class _TimelineItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isPending = step.status.toLowerCase() == 'pending' || step.status.isEmpty;
...
class _Content extends StatelessWidget {
  ...
  Widget _titleRow() {
    final showNewStepHint = _isNewStepAwaiting();
    return Row(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        AutoSizeText(
        ...
        // ─── Hint pill: ขั้นใหม่ที่ยังไม่ได้เริ่มทำ ───
        if (showNewStepHint) ...[
          const SizedBox(width: 6),
          Flexible(
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(LaRadius.pill),
                border: Border.all(
                  color: LaColors.statusPendingFg.withOpacity(.35),
                  width: 1,
                ),
              ),
              child: Row(
                ...
                children: [
                  Icon(Icons.bolt_rounded,
                      size: 12, color: LaColors.statusPendingFg),
                  const SizedBox(width: 4),
                  Flexible(
                    child: Text(
                      'เปิดรอบใหม่ รอเข้าหน้าใส่ข้อพิพาท',
                      maxLines: 1,
  ...
  /// ขั้นใหม่ที่ "เพิ่งเปิด" และยังไม่ได้เริ่มทำ
  /// เงื่อนไข: status == pending + isCurrent + canAct + ยังไม่มี actedBy
  bool _isNewStepAwaiting() {
    final s = step.status.toLowerCase();
    final isPending = s.isEmpty ||
        s.contains('pending') ||
        s.contains('รอ') ||
        s.contains('progress');
    final noActedBy = step.actedBy == null || (step.actedBy ?? '').trim().isEmpty;
    return step.isCurrent && step.canAct && isPending && noActedBy;
  }
  ...
  Widget _remarkBox() {
    // ─── กล่องเขียวพิเศษ ───
    // ถ้า remark มีคำว่า "start collecting data" / "กำลังเก็บ" / "เริ่มเก็บ"
    // → ใช้กล่องเขียว highlight (ระบบกำลังเก็บข้อมูล)
      ...
      child: const Row(
        ...
        children: [
          Icon(Icons.touch_app_rounded,
              size: 12, color: LaColors.primaryDark),
          SizedBox(width: 4),
          Text(
            'คุณสามารถอนุมัติ/ปฏิเสธขั้นนี้ได้',
            style: TextStyle(
              color: LaColors.primaryDark,
              fontSize: 11,
              fontWeight: FontWeight.w600,
