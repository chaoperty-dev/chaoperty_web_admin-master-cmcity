// ============================================================================
// verify_documents_service.dart
// ============================================================================
// Service — โหลด "รายการเอกสารที่ต้องแนบ" + อัปโหลด/ลบไฟล์แนบ
//
// เป็น service อิสระของ License Verify Detail Page
...
/// Service สำหรับเรียก API จัดการเอกสารแนบของ License Verify
class VerifyDocumentsService {
  VerifyDocumentsService();

  /// โหลดรายการเอกสาร (พร้อม attachments ที่แนบแล้ว) ตาม request uuid
  /// คืน List<LicenseverifyDocument> (แต่ละ doc มี attachments ครบ)
  Future<List<LicenseverifyDocument>> fetchDocuments(String requestUuid) async {
    ...
    try {
      ...
      if (documentsRaw is! List) return <LicenseverifyDocument>[];

      return documentsRaw
          .whereType<Map<String, dynamic>>()
          .map<LicenseverifyDocument>((e) => LicenseverifyDocument.fromJson(e))
          .toList();
    } catch (_) {
      return <LicenseverifyDocument>[];
    }
  ...
  /// อัปเดตสถานะการตรวจสอบของ attachment (approved / rejected / needs_update)
  Future<bool> updateAttachmentReviewStatus({
    required String requestUuid,
    required String attachmentUuid,
    ...
    // ignore: avoid_print
    print(
        '📡 [VerifyDocumentsService] updateAttachmentReviewStatus requestUuid=$requestUuid attachmentUuid=$attachmentUuid status=$status');
    try {
      final resp = await Post_ReviewsFlowAttachMents(
        requestUuid: requestUuid,
        attachmentsUuid: attachmentUuid,
        staTus: status,
      ...
      // ignore: avoid_print
      print(
          '📡 [VerifyDocumentsService] updateAttachmentReviewStatus result=$ok statusCode=${resp?.statusCode}');
      return ok;
    } catch (e) {
      // ignore: avoid_print
      print('📡 [VerifyDocumentsService] updateAttachmentReviewStatus error=$e');
      return false;
    }
  }
  ...
  ///
  /// - ถ้า [LicenseverifyAttachment.filePath] เป็น full URL → ใช้ตรงๆ พร้อม auth header
  /// - ถ้าเป็น relative path → ต่อกับ domain_v1
  /// - ถ้ามี [LicenseverifyAttachment.uuid] → ใช้ endpoint
  ///   `/request-snapshot-attachments/{uuid}/preview` ซึ่งรองรับ auth
  Future<Uint8List?> fetchAttachmentBytes(LicenseverifyAttachment att) async {
