// ============================================================================
// license_announce_add_page.dart
// ============================================================================
// Full-page "เพิ่มประกาศ" — เหมือน edit page แต่ไม่มี initial data
// Reuse form structure เดียวกับ edit page
// ============================================================================
...
class LicenseAnnounceAddPage extends StatefulWidget {
  const LicenseAnnounceAddPage({super.key});

  @override
  State<LicenseAnnounceAddPage> createState() => _LicenseAnnounceAddPageState();
}

class _LicenseAnnounceAddPageState extends State<LicenseAnnounceAddPage> {
  ...
  void initState() {
    ...
    _sdate = TextEditingController(text: f.format(today));
    _edate = TextEditingController(
        text: f.format(today.add(const Duration(days: 7))));
    _adate = TextEditingController(text: f.format(today));
    _cdateStart = TextEditingController(text: f.format(today));
    _cdateEnd = TextEditingController(
        text: f.format(today.add(const Duration(days: 365))));
  ...
  Future<void> _save() async {
    ...
    final vm = context.read<LicenseAnnounceViewModel>();
    final zones = vm.zones;
    // สร้าง zone data (id + pn + subzone_id) จากเซ็ตที่เลือก
    ...
    final ok = await vm.addAnnouncement(
    ...
    if (ok) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('เพิ่มประกาศสำเร็จ'),
        behavior: SnackBarBehavior.floating,
      ));
      if (Navigator.of(context).canPop()) Navigator.of(context).pop(true);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('เพิ่มประกาศล้มเหลว'),
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            ...
            _AddFooter(submitting: _submitting, onSave: _save),
          ],
        ),
      ),
    );
...
class _AddFooter extends StatelessWidget {
  ...
  const _AddFooter({required this.submitting, required this.onSave});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 6),
          Text(
            submitting ? 'กำลังเพิ่ม...' : 'กรอกข้อมูลให้ครบถ้วนก่อนกดบันทึก',
