...
class LicenseAttachViewModel extends ChangeNotifier {
  ...
  // ---------- Status filter ----------
  /// รายการ status ทั้งหมดที่ filter ได้ — delegate to central mapper
  /// (null = "ทั้งหมด" — ไม่ส่ง key ให้ backend)
  static List<String> get statusOptions => LicenseStatusLabels.options;

  /// ป้ายภาษาไทยสำหรับ status — delegate to central mapper
  ...
  /// → sync ไปที่ global store → จะ trigger _onZoneStoreChanged ที่จัดการ reset zone + reload
  Future<void> onSubZoneChanged(String? value) async {
    if (value == null) return;
    _zoneStore.setLicenseSubZone(value);
  }

  /// ผู้ใช้เลือก "โซน" → sync ไปที่ global store → จะ trigger _onZoneStoreChanged ที่ reload
