...
class Account_Cancel_PayVerifi extends StatefulWidget {
  @override
  _Account_Cancel_PayVerifiState createState() =>
      _Account_Cancel_PayVerifiState();
}

class _Account_Cancel_PayVerifiState extends State<Account_Cancel_PayVerifi> {
  //-------------------------------------->
...
/////////--------------------------------------------->
  ...
  Future<Null> red_Trans_bill_Verifi() async {
    ...
    String url = (zone == null || zone == '0')
        ? '${MyConstant().domain}/GC_bill_pay_BC_VerifiCancel.php?isAdd=true&ren=$ren&mont_h=$MONTH_Now&yea_r=$YEAR_Now&serpang=$sertype&serzone=$zone&date_type=$Date_Typepay'
        : '${MyConstant().domain}/GC_billZone_pay_BC_VerifiCancel.php?isAdd=true&ren=$ren&mont_h=$MONTH_Now&yea_r=$YEAR_Now&serpang=$sertype&serzone=$zone&date_type=$Date_Typepay';
    try {
      var response = await http.get(Uri.parse(url));
    ...
    if (TransReBillModels.isNotEmpty) {
      ...
      setState(() {
        data = List.generate(TransReBillModels.length, (index) {
          ...
          final date_loc = (_TransReBillModels[index].pay_by.toString() == 'LP')
          ...
          final zn = TransReBillModels[index].zn == null
              ? '${TransReBillModels[index].znn}'
              : '${TransReBillModels[index].zn}';
          final ln = TransReBillModels[index].ln == null
              ? '${TransReBillModels[index].room_number}'
              : '${TransReBillModels[index].ln}';

          final cname = TransReBillModels[index].cname == null
              ? '${TransReBillModels[index].remark}'
              : '${TransReBillModels[index].cname}';
          final sname = TransReBillModels[index].sname == null
              ? '${TransReBillModels[index].remark}'
          ...
          final type = TransReBillModels[index].type == null
              ? ''
              : '${TransReBillModels[index].type}';
          final ref1 = TransReBillModels[index].ref1 == null
              ? ''
              : '${TransReBillModels[index].ref1}';
          final ref2 = TransReBillModels[index].ref2 == null
              ? ''
              : '${TransReBillModels[index].ref2}';
          final ref4 = TransReBillModels[index].ref4 == null
              ? ''
              : '${TransReBillModels[index].ref4}';
          final pay_by = (TransReBillModels[index].pay_by.toString() == 'W')
              ? 'Web Admin (${TransReBillModels[index].pay_by})'
              : (TransReBillModels[index].pay_by.toString() == 'U')
                  ? 'Web User (${TransReBillModels[index].pay_by})'
                  : (TransReBillModels[index].pay_by.toString() == 'LP')
                      ? 'Web Market (${TransReBillModels[index].pay_by})'
                      : (TransReBillModels[index].pay_by.toString() == 'LP')
                          ? 'Handheld (${TransReBillModels[index].pay_by})'
                          : 'Unknow ?? (${TransReBillModels[index].pay_by})';
          final pay_type = (TransReBillModels[index].pay_by.toString() == 'W')
              ? 'ชำระค่าบริการ'
              : (TransReBillModels[index].pay_by.toString() == 'U')
                  ? 'ชำระค่าบริการ'
                  : (TransReBillModels[index].pay_by.toString() == 'LP')
                      ...
                      : (TransReBillModels[index].pay_by.toString() == 'LP')
                          ? 'ชำระค่าบริการ'
                          : 'ไม่ทราบ ??';
          final remark = (TransReBillModels[index].remark == null)
              ? '-'
              : '${TransReBillModels[index].remark}';

          return {
            "index": "$index",
            if (where_ac9_3("0") == false) "เลขที่สัญญา": "$cid",
            if (where_ac9_3("1") == false) "วันที่ทำรายการ": "$daterec",
            if (where_ac9_3("2") == false) "วันที่จอง-ล็อกเสียบ": "$date_loc",
            if (where_ac9_3("3") == false) "วันที่รับชำระ": "$pdate",
            if (where_ac9_3("4") == false) "เลขที่ใบเสร็จ": "$docno",
            if (where_ac9_3("5") == false) "เลขที่ใบวางบิล": "$doc_inv",
            if (where_ac9_3("6") == false) "โซนพื้นที่": "$zn",
            if (where_ac9_3("7") == false) "รหัสพื้นที่": "$ln",
            if (where_ac9_3("8") == false) "ชื่อร้านค้า": "$sname",
            if (where_ac9_3("9") == false) "ชื่อผู้ติดต่อ": "$cname",
            if (where_ac9_3("10") == false) "จำนวนเงิน": "$total",
            if (where_ac9_3("11") == false) "รูปแบบชำระ": "$type",
            if (where_ac9_3("12") == false) "รหัสอ้างอิง": "$ref1",
            if (where_ac9_3("13") == false) "Ref1": "$ref2",
            if (where_ac9_3("14") == false) "Ref2": "$ref4",
            if (where_ac9_3("15") == false) "ทำรายการ": "$pay_by",
            if (where_ac9_3("16") == false) "ประเภท": "$pay_type",
            if (where_ac9_3("17") == false) "เหตุผล": "$remark",
          };
        });
        filteredData = data;
      });
                      ...
                      child: Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton2<String>(
                            ...
                            items: ac9_3.asMap().entries.map((entry) {
                              ...
                              return DropdownMenuItem<String>(
                                ...
                                child: StatefulBuilder(
                                  builder: (context, menuSetState) {
                                    ...
                                    return InkWell(
                                      onTap: () {
                                        int selectedIndex = ac9_3.indexWhere(
                                            (items) =>
                                                items["ser"] == item["ser"]);
                                        ...
                                        setState(() {
                                          if (item["st"]! == '1') {
                                            ac9_3[selectedIndex]["st"] = '0';
                                          } else {
                                            ac9_3[selectedIndex]["st"] = '1';
                                          }
                                        });
                                        AddDaTa();
