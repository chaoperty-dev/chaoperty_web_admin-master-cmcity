...
// Protecting the Worksheet by using a Password
    ...
    sheet.getRangeByName('G4').setText('โซนพื้นที่');
    sheet.getRangeByName('H4').setText('รหัสพื้นที่');
    sheet.getRangeByName('I4').setText('ขนาดพื้นที่(ต.ร.ม.)');
    sheet.getRangeByName('J4').setText('ระยะเวลาการเช่า');
    ...
    for (int i = 0; i < expModels.length; i++) {
      sheet.getRangeByName('${columns[13 + i]}1').cellStyle = globalStyle22;
      sheet.getRangeByName('${columns[13 + i]}2').cellStyle = globalStyle22;
      sheet.getRangeByName('${columns[13 + i]}3').cellStyle = globalStyle22;

      sheet.getRangeByName('${columns[13 + i]}4').cellStyle = globalStyle1;
      sheet.getRangeByName('${columns[13 + i]}4').columnWidth = 35;
      sheet
          .getRangeByName('${columns[13 + i]}4')
    ...
    for (int i = 0; i < teNantModels.length; i++) {
      ...
      sheet.getRangeByName('K${i + 5}').setText(
            teNantModels[i].sdate == null
                ...
                : DateFormat('dd-MM-yyyy')
                    .format(DateTime.parse('${teNantModels[i].sdate} 00:00:00'))
                    .toString(),
          );
      sheet.getRangeByName('L${i + 5}').setText(
            teNantModels[i].ldate == null
                ? ''
                : DateFormat('dd-MM-yyyy')
                    .format(DateTime.parse('${teNantModels[i].ldate} 00:00:00'))
                    .toString(),
          );
      ...
      for (int index2 = 0; index2 < expModels.length; index2++) {
        ...
        String unit = (dataList.isEmpty)
            ...
            : dataList
                    ...
                    .map((element) => element['unit_exp']?.toString() ?? '0.00')
                    .isNotEmpty
                ? dataList
                    .whereType<Map<String, dynamic>>()
                    .where((element) =>
                    ...
                    .map((element) => element['unit_exp']?.toString() ?? '0.00')
                    .join(',')
                : ''; // Default value if no matches found
        ...
        sheet
            ...
            .setText((data_ok == 0)
                ? ''
                : (etype.toString() == 'F')
                    ? '$unit ${nFormat.format(amt)} / บาท'
                    : (ele_ty.toString() == '0' || (ele_ty.toString() == '0.00')
                        ? (qty.toString() == '0.00' || qty == '0')
                            ? '$unit ${nFormat.format(total)} / งวด'
                            : '$unit ${nFormat.format(double.tryParse(qty) ?? 0.0)} / หน่วย'
                        : '$unit อัตราพิเศษ'));
      }
    }
