...
class Excgen_ChaoAreaLocReport {
  static void exportExcel_ChaoAreaLocReport(
    context,
    NameFile_,
...
// Protecting the Worksheet by using a Password
    ...
    sheet.getRangeByName('B2').setText(
          'ถึงวันที่ : $LDatex_area',
        );
    sheet.getRangeByName('C2').setText(
          'ทั้งหมด: ${areaModels.length}',
        );
    sheet.getRangeByName('F2').setText(' ข้อมูลอัพเดต ณ : ${DateTime.now()} ');
    ...
    sheet.getRangeByName('E4').columnWidth = 50;
    sheet.getRangeByName('F4').columnWidth = 50;
    sheet.getRangeByName('G4').columnWidth = 50;
    ...
    sheet.getRangeByName('D4').setText('เลขที่สัญญา/ใบเสนอราคา');
    sheet.getRangeByName('E4').setText('เลขที่จอง(ล็อกเสียบ)');

    sheet.getRangeByName('F4').setText('วันที่ทำรายการ(ล็อกเสียบ)');
    sheet.getRangeByName('G4').setText('ชื่อผู้เช่า/ชื่อผู้จอง(ล็อกเสียบ)');
    // sheet.getRangeByName('H4').setText('วันที่เริ่มต้น(ล็อกเสียบ)');
    // sheet.getRangeByName('I4').setText('ชื่อผู้เช่า/ชื่อผู้จอง');
    // sheet.getRangeByName('J4').setText('ประเภท');
    // sheet.getRangeByName('K4').setText('สถานะ');
    // sheet.getRangeByName('L4').setText('วันที่ทำรายการ(ล็อกเสียบ)');
    // sheet.getRangeByName('M4').setText('เวลาที่ทำรายการ(ล็อกเสียบ)');
    // sheet.getRangeByName('N4').setText('หลักฐาน(ล็อกเสียบ)');
    int index1 = 0;
    for (int index = 0; index < areaModels.length; index++) {
      ...
      //       (areaModels[index].cid == null && areaModels[index].docno != null)
      //           ? areaModels[index].sdate_q == null
      //               ? ''
      //               : '${areaModels[index].sdate_q}'
      //           : areaModels[index].sdate == null
      //               ? ''
      ...
      sheet.getRangeByName('D${index + 5}').setText(
            (areaModels[index].cid == null && areaModels[index].docno != null)
                ...
                : (areaModels[index].cid == null)
                    ? ''
                    : '${areaModels[index].cid}',
          );
      sheet.getRangeByName('E${index + 5}').setText(
            areaModels[index].exp_array == null
                ? '-'
                : (() {
                    var decoded =
                        json.decode(areaModels[index].exp_array.toString());

                    // Ensure that the decoded value is a List
                    if (decoded is List) {
                      return decoded
                          .whereType<Map<String, dynamic>>()
                          .map((element) => element['docno'].toString())
                          .join(', ');
                    } else {
            ...
            areaModels[index].exp_array == null
                ...
                : (() {
                    ...
                    if (decoded is List) {
                      return decoded
                          .whereType<Map<String, dynamic>>()
                          .map((element) => element['sname'].toString())
                          .join(', ');
                    } else {
                      return '-'; // In case it's not a List, return '-'
                    }
      ...
      // sheet.getRangeByName('G${index + 5}').setText(
      //       areaModels[index].docno_book == null
      //           ? ''
      //           : '${areaModels[index].docno_book}',
      //     );
      ...
      //       (areaModels[index].cid == null && areaModels[index].docno != null)
      //           ? areaModels[index].sname_q == null
      //               ? (areaModels[index].cname_q == null)
      //                   ? ''
      //                   : '${areaModels[index].cname_q}'
      //               : '${areaModels[index].sname_q}'
      //           : areaModels[index].sname == null
      //               ? (areaModels[index].cname == null)
      //                   ? ''
      ...
      //       (areaModels[index].cid == null && areaModels[index].docno != null)
      //           ? areaModels[index].stype_q == null
      //               ? ''
      //               : '${areaModels[index].stype_q}'
      //           : areaModels[index].stype == null
      //               ? ''
      ...
      //       areaModels[index].datex == null ? '' : '${areaModels[index].datex}',
      //     );
      // sheet.getRangeByName('M${index + 5}').setText(
      //       areaModels[index].timex == null ? '' : '${areaModels[index].timex}',
      //     );
      // sheet.getRangeByName('N${index + 5}').setText(
      //       areaModels[index].img == null ? '' : 'พบหลักฐาน',
      //     );
      // sheet.getRangeByName('K${index + 5}').setText(
...
/////////////////////////////////------------------------------------------------>
    // sheet.getRangeByName('G${areaModels.length + 5}').setText('รวมทั้งหมด: ');
    // sheet
    //     .getRangeByName('H${areaModels.length + 5}')
    //     .setFormula('=SUM(H5:H${areaModels.length + 4})');

    // sheet.getRangeByName('G${areaModels.length + 5}').cellStyle = globalStyle7;
    // sheet.getRangeByName('H${areaModels.length + 5}').cellStyle = globalStyle7;
...
/////////////////////////////////------------------------------------------------>
    ...
    String path = await FileSaver.instance.saveFile(
        "รายงานข้อมูลพื้นที่ล็อกเสียบ(วันที่:${SDatex_area}To${LDatex_area}โซน:$Value_Chang_Zone_Area)",
        data,
        "xlsx",
        mimeType: type);
    log(path);
