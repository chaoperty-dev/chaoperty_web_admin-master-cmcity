...
class Excgen_BillPayMonRent2Report_Choice {
  static void exportExcel_BillPayMonRent2Report_Choice(
    ...
    sheet.name = 'รายงานค่าเช่าประจำเดือนฉบับย่อ';
...
////////////-------------------------------------------------------->
    ...
    sheet.getRangeByName('A1').setText(
          (Value_Chang_Zone_billpayMon2 == null)
              ? 'รายงานค่าเช่า-ฉบับย่อ  (กรุณาเลือกโซน)'
              : 'รายงานค่าเช่า-ฉบับย่อ (โซน : $Value_Chang_Zone_billpayMon2)',
        );
    sheet
        .getRangeByName('A2')
        .setText('เดือนที่รับชำระ ${Mon_billpay_Mon2} ${YE_billpay_Mon2}');
    ...
    sheet.getRangeByName('A5').setText('ทั้งหมด :${billpay_Mon2.length}');
...
// Protecting the Worksheet by using a Password
    ...
    sheet.getRangeByName('G6').setText('เลขที่ล็อค');
    sheet.getRangeByName('H6').setText('เลขประจำตัวผู้เสียภาษี');
    sheet.getRangeByName('I6').setText(' จำนวนเงิน');
    ...
    for (int index = 0; index < billpay_Mon2.length; index++) {
      ...
      sheet.getRangeByName('B${index + 7}').setValue(
            (billpay_Mon2[index].pdate == null ||
                    billpay_Mon2[index].pdate.toString() == '')
                ? null
                : DateTime.parse('${billpay_Mon2[index].pdate}'),
          );
...
/////////---------------------------->
    ...
    String path = await FileSaver.instance.saveFile(
        (Value_Chang_Zone_billpayMon2 == null)
            ? 'รายงานค่าเช่าฉบับย่อ(กรุณาเลือกโซน)'
            : 'รายงานค่าเช่าฉบับย่อ(โซน : $Value_Chang_Zone_billpayMon2)',
        data,
        "xlsx",
        mimeType: type);
    log(path);
