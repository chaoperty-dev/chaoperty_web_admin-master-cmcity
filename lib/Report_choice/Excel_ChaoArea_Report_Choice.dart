...
class Excgen_ChaoAreaReport_Choice {
  static void exportExcel_ChaoAreaReport_Choice(
      expModels,
      context,
      NameFile_,
...
// Protecting the Worksheet by using a Password
    ...
    sheet.getRangeByName('H2').setText(' ข้อมูล ณ วันที่: ${DateTime.now()}');
    ...
    sheet.getRangeByName('B4').setText('รหัสโซนพื้นที่');
    ...
    sheet.getRangeByName('H4').setText('ประภทสัญญา');
    sheet.getRangeByName('I4').setText('วันที่เริ่มสัญญา');
    sheet.getRangeByName('J4').setText('วันที่สิ้นสุดเริ่มสัญญา');
    ...
    for (int index = 0; index < areaModels.length; index++) {
      ...
      sheet.getRangeByName('B${index + 5}').setText(
            (areaModels[index].zn == null ||
                    areaModels[index].zn == '' ||
                    areaModels[index].zn == 'null')
                ? ''
                : (areaModels[index].zn!.split('_')[0].length <= 4)
                    ? 'CMN0${areaModels[index].zn!.split('_')[0]}'
                    : 'CMN${areaModels[index].zn!.split('_')[0]}',
          );
      sheet.getRangeByName('C${index + 5}').setText(
            (areaModels[index].zn == null) ? '' : '${areaModels[index].zn}',
          );
      sheet.getRangeByName('D${index + 5}').setText(
            (areaModels[index].ln == null) ? '' : '${areaModels[index].ln}',
          );
      sheet.getRangeByName('E${index + 5}').setNumber(
            (areaModels[index].area == null ||
                    areaModels[index].area == '' ||
                    areaModels[index].area == 'null')
                ? 0.00
                : double.parse(areaModels[index].area!),
          );
      sheet.getRangeByName('F${index + 5}').setNumber(
            (areaModels[index].rent == null ||
      ...
      sheet.getRangeByName('G${index + 5}').setText(
            (areaModels[index].cid == null ||
                    areaModels[index].cid.toString() == 'null')
                ? ''
      ...
      try {
        sheet.getRangeByName('I${index + 5}').cellStyle.numberFormat =
            'dd-MM-yyyy';
        sheet.getRangeByName('I${index + 5}').setValue(
              (areaModels[index].sdate == null)
                  ? null
                  : DateTime.parse('${areaModels[index].sdate} 00:00:00'),
            );
      } catch (e) {
      ...
      try {
        sheet.getRangeByName('J${index + 5}').cellStyle.numberFormat =
            'dd-MM-yyyy';
        sheet.getRangeByName('J${index + 5}').setValue(
      ...
      sheet.getRangeByName('L${index + 5}').setText(
            (areaModels[index].stype == null ||
                    areaModels[index].stype.toString() == 'null')
                ? ''
    ...
    String path = await FileSaver.instance.saveFile(
        "ExclusiveE_รายงานข้อมูลพื้นที่เช่า", data, "xlsx",
        mimeType: type);
    log(path);
