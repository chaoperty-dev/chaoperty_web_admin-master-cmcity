...
class _SignaturePad_CMMState extends State<SignaturePad_CMM> {
  ...
  Future<void> _check_main() async {
    ...
    if (value.toString() == 'true') {
      // ✅ consume flag ก่อน — ป้องกันเปิด dialog bulk-approve ซ้ำเมื่อเข้าหน้านี้ครั้งถัดไป
      await SecurePrefs.setEncrypted(SecurePrefsType.approverAll, 'false');

      final zn = await SecurePrefs.getDecrypted(SecurePrefsType.approverAllZn);
      final subZn =
          await SecurePrefs.getDecrypted(SecurePrefsType.approverAllSubZn);
      final orderBy =
          await SecurePrefs.getDecrypted(SecurePrefsType.approverAllOrderBy);
      final sortDir =
          await SecurePrefs.getDecrypted(SecurePrefsType.approverAllSortDir);

      // ✅ guard: ห้ามยิง API ถ้าไม่มี zn (bulk-approve ต้องมีโซนเสมอ)
      if (zn == null || zn.isEmpty) {
        if (mounted) {
          await Dialog_error(
              context, 'ไม่พบโซนพื้นที่สำหรับอนุมัติทั้งหมด กรุณาเลือกโซนก่อน');
        }
        _bootstrap();
        return;
      }
      ...
      final res = await read_GC_ApprovalsRoles(
        ...
        perPage: 100,
        zn: zn,
        orderBy: (orderBy == null || orderBy.isEmpty) ? null : orderBy,
        sortDir: (sortDir == null || sortDir.isEmpty) ? null : sortDir,
      );
      setState(() {
        reviewModels = res.data ?? [];
        _bulkZone = zn;
        _bulkSubZone = (subZn == null || subZn.isEmpty) ? null : subZn;
      });
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_bulkSubZone != null)
                            Text('โซน: $_bulkSubZone',
                                style: Theme.of(context)
                                    .textTheme
                                    .bodySmall
                                    ?.copyWith(
                                        ...
                                        child: SizedBox(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              // 🔹 body — fixed height so Expanded widgets have bounded space
                                              Expanded(
                                                child: ClipRRect(
                                                  borderRadius:
                                                      const BorderRadius
