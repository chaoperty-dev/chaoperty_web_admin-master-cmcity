...
class LicenseApproveViewModel extends ChangeNotifier {
  LicenseApproveViewModel({
    required LicenseApproveConfig config,
    LicenseApproveService? service,
  })  : _config = config,
        _service = service ?? LicenseApproveService() {
    _loadInitial();
  }

  final LicenseApproveConfig _config;
  final LicenseApproveService _service;

  // ---------- Event channel ----------
  final StreamController<LicenseApproveEvent> _eventController =
      StreamController<LicenseApproveEvent>.broadcast();
  Stream<LicenseApproveEvent> get events => _eventController.stream;
  ...
  void onViewRequest(ReviewModel model) {
    ...
    _eventController.add(
      LicenseApproveNavigateEvent('อนุมัติคำขอ', routeData: uuid),
    );
  }
  ...
  void _emitError(String msg) {
    _eventController.add(LicenseApproveErrorEvent(msg));
  }
