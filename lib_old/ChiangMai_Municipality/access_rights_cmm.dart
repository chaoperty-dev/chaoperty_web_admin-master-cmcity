...
class _AccessRights_CMMState extends State<AccessRights_CMM> {
  ...
  Future<void> red_Permission() async {
    ...
    if (rolesall.isNotEmpty) {
      ...
      //print('red_Permission positionsall: ${positionsall.length}');
      //print('red_Permission rolesall: ${rolesall.length}');
      //print('red_Permission rolesAllModel: ${rolesAllModel.length}');
      //print('red_Permission _list: ${_list.length}');
      //print('red_Permission checkedStates: ${checkedStates.length}');
    } else {
...
//////////////------------------------------------------------------>
  ...
  Future<void> red_User() async {
    ...
    final result = await read_GC_UserCMM('urlCustom');
        ...
        data = List.generate(usermodel.length, (index) {
          ...
          return {
            ...
            "สิทธิ์การเข้าถึง": user.roles != null && user.roles!.isNotEmpty
                ? (user.roles!.map((role) => role.nameTh).toList()..sort())
                    .join(', ')
          ...
          Row(
            children: [
              ...
              Padding(
                ...
                child: InkWell(
                  onTap: () async {
                    PositionsAll? Dropdown_initialItemx;
                    setState(() {
                      // rolesAllModel = [];
                      // _list = [];
                      checkedStates = [];
                      Dropdown_initialItem = Dropdown_initialItemx;
                    });
                    setState(() {
                      // positionsAllModel.clear();
                      positionsId = null;
                    ...
                    //print('เพิ่มผู้ใช้ ${rolesAllModel.length}');
                    addAcListTitle();
              ...
              child: Container(
                  ...
                  child: ConstrainedBox(
                    constraints: BoxConstraints(
                      maxWidth:
                          (maxAllowedWidth < 1300) ? 1400 : maxAllowedWidth,
                                  ...
                                  : ListView.builder(
                                      ...
                                      itemBuilder:
                                        ...
                                        return List_Material(index,
                                            columnHeaders, row, columnToCheck);
                                      },
                                    ),
                        ),
                      ],
                      ...
                      return column.toString() == 'เลขที่สัญญา' ||
                        ...
                        : Expanded(
                            ...
                            child: AutoSizeText(
                              minFontSize: 12,
                              maxFontSize: 16,
                              maxLines: column.toString() == 'สิทธิ์การเข้าถึง'
                                  ? 3
                                  : 1,
                              row[column]?.toString() ?? '',
                              textAlign: (columnHeaders.any((columnx) {
                  ...
                  onSelected: (v) async {
                    if (v == 'edit') {
                      ...
                      setState(() {
                        ...
                        controller(7).text = '';
                        controller(8).text = '';
                        controller(9).text = profile['prepostion'] ?? '';
                      ...
                      final roleList = data['roles'] as List;
                      Uint8List? sigBytes;
                      if (jsonRes['data']['signatures'] != null &&
                          jsonRes['data']['signatures'].isNotEmpty &&
                          jsonRes['data']['signatures'][0]['uuid'] != null) {
                        final sigResp = await img_signatureUuid(
                            signatureUuid:
                                '${jsonRes['data']['signatures'][0]['uuid']}');
                      ...
                      setState(() {
                        checkedStates =
                            List.filled(rolesAllModel.length, false);
                        ...
                        // signaturesUrl = (jsonRes['data']['signatures'] !=
                        //             null &&
                        //         jsonRes['data']['signatures'].isNotEmpty &&
                        //         jsonRes['data']['signatures'][0]['uuid'] !=
                        //             null)
                      ...
                      Future.delayed(const Duration(milliseconds: 300), () {
                        Edit_userAcess(
                            'แก้ไขข้อมูลผู้ใช้', indexX, 2, usersUuid);
                      });
                    } else if (v == 'signature') {
                      await Dia_log1(context);
                      final indexX = int.tryParse('${row['index']}');
                      ...
                      Future.delayed(const Duration(milliseconds: 300), () {
                        Edit_userSignatures(
                            'แก้ไขลายเซ็นผู้ใช้', indexX, 1, usersUuid);
                      });
                    }
                  },
                  itemBuilder: (_) => const [
                    PopupMenuItem(value: 'edit', child: Text('แก้ไขข้อมูล')),
                    PopupMenuItem(
                        value: 'signature', child: Text('จัดการลายเซ็น')),
                  ],
                  child: Center(
                          ...
                          Row(
                            children: [
                              Expanded(
                                ...
                                child: Padding(
                                    padding: const EdgeInsets.all(4.0),
                                    child: (positionsId == null)
                                        ? SizedBox(
                                            width: 100,
                                            height: 260,
                                            child: Center(
                                              child: Translate.TranslateAndSetText(
                                                  ' ** กรุณาระบุลำดับรายมือชื่อก่อน..!!',
                                                  Colors.red,
                                                  TextAlign.left,
                                                  null,
                                                  Font_.Fonts_T,
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          for (int i = 0; i < checkedStates.length; i++) {
                            if (checkedStates[i]) {
                              selectedSer.add(rolesAllModel[i].id);
                            }
                          }

                          selectedSer.sort();
                          //print('✅ Checked ser list: ${selectedSer.join(',')}');
                          final prefix =
                              data_title[0]['controller'].text.trim();
                          final firstName =
                          ...
                          final prepostionText =
                              data_title[9]['controller'].text;
                          ...
                          if (response?.statusCode == 201 ||
                            ...
                            //     .bytesToString(); // ✅ อ่านครั้งเดียว
                            // //print('✅ Success: $responseBody');
                            // Dialog_error(context, '❌ ล้มเหลว');

                            Navigator.of(context).pop();

                            // Future.delayed(const Duration(seconds: 1), () {
                            //   Dialog_success(context, 'สำเร็จ');
                            // });
                          } else {
                            Dialog_error(context, '❌ ล้มเหลว');
                            // final responseBody = await response!.stream
