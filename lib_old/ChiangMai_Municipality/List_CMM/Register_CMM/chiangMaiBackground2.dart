...
class ChiangMaiBackground2 extends StatefulWidget {
  const ChiangMaiBackground2({super.key});

  @override
  State<ChiangMaiBackground2> createState() => _ChiangMaiBackground2State();
}

class _ChiangMaiBackground2State extends State<ChiangMaiBackground2>
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: _ChiangMaiPainter2(animation: _ctrl),
      size: Size.infinite,
    );
  }
}

class _ChiangMaiPainter2 extends CustomPainter {
  final Animation<double> animation;
  _ChiangMaiPainter2({required this.animation}) : super(repaint: animation);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // เวลาอนิเมชัน (0..1 วิ่งไปเรื่อย ๆ)
    final t = animation.value * 2 * math.pi;

    // โคมแต่ละดวงมี phase/strength ต่างกันเล็กน้อยให้ดูเป็นธรรมชาติ
    _drawLanternAnimated(canvas, size,
        base: Offset(size.width * 0.22, size.height * 0.28),
        w: 26,
        h: 38,
        t: t,
        phase: 0.0,
        amp: 5);
    _drawLanternAnimated(canvas, size,
        base: Offset(size.width * 0.35, size.height * 0.22),
        w: 22,
        h: 32,
        t: t,
        phase: 1.2,
        amp: 6);
    _drawLanternAnimated(canvas, size,
        base: Offset(size.width * 0.68, size.height * 0.18),
        w: 28,
        h: 42,
        t: t,
        phase: 2.4,
  ...
  // ========== เวอร์ชันวาดโคม + อนิเมชัน ==========
  void _drawLanternAnimated(
    Canvas canvas,
    Size size, {
    ...
    required double amp, // ระยะลอย (px)
  }) {
    // ลอยขึ้นลง + แกว่งซ้ายขวาเล็กน้อย
    final dy = math.sin(t + phase) * amp; // ขึ้นลง
    final dx = math.sin(t * 0.8 + phase) * (amp * 0.35); // ซ้ายขวาเบา ๆ
    final sway = math.sin(t * 0.7 + phase) * 0.03; // หมุนเบา ๆ (ราว 1.7°)

    final center = base.translate(dx, dy);

    // หมุนรอบจุดกึ่งกลางเล็กน้อย
    canvas.save();
    canvas.translate(center.dx, center.dy);
    canvas.rotate(sway);
    canvas.translate(-center.dx, -center.dy);

    // เรียกตัวเดิม (แก้ให้รับ center)
    _drawLantern(canvas, size, center, w, h, t: t, phase: phase);

    canvas.restore();
  }

  // ---------- Yi Peng lanterns (เพิ่มพารามิเตอร์ t/phase สำหรับ glow) ----------
  void _drawLantern(
    Canvas canvas,
    Size size,
    Offset center,
    ...
    // soft glow (กระพริบจาง ๆ)
    final flicker = 0.25 + 0.1 * math.sin(t * 1.3 + phase); // 0.15..0.35
    final glowPaint = Paint()
      ..color = const Color(0xFFFFE08A).withOpacity(flicker)
  ...
  // ====== ส่วนอื่นของคุณ (Sky/Mountains/Chedi/Trap) คงเดิม ======
  // ... _drawSkyGradient / _drawMountains / _drawChediSilhouette / _trap ...
  // (คงฟังก์ชันของคุณเดิมทั้งหมด ไม่ต้องแก้)
  // ===============================================================
