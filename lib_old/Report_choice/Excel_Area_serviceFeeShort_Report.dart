...
/////////------------------------------------------->sheet-1
    ...
    // sheet.getRangeByName('A2').setText('เดือนภาษี ${Mon_Pe_Mon} ${YE_Pe_Mon}');
    sheet.getRangeByName('A2').setText(
        'ชื่อสถานประกอบการ บริษัท ชอยส์มินิสโตร์ จำกัด เลขประจำตัวผู้เสียภาษี 0-1055-31085-43-4');
    // sheet.getRangeByName('A3').setText('ชื่อสถานประกอบการ เซเว่นอีเลฟเว่น');
    sheet.getRangeByName('A3').setText('ทั้งหมด :${teNantModels.length}');
    sheet.getRangeByName('G3').setText('อัพเดต ณ :${DateTime.now()}');
/////////------------------------------------------->sheet-2
...
/////////------------------------------------------->
    ...
    sheet.getRangeByName('G4').columnWidth = 60;
    // sheet.getRangeByName('H4').columnWidth = 30;
    // sheet.getRangeByName('I4').columnWidth = 18;

    sheet.getRangeByName('A4').setText('ลำดับที่');
    sheet.getRangeByName('B4').setText('จำนวนสัญญา');
    sheet.getRangeByName('C4').setText('รหัสโซน');
    sheet.getRangeByName('D4').setText('โซนหลัก');
    sheet.getRangeByName('E4').setText('โซนรอง');
    sheet.getRangeByName('F4').setText('ล็อก');
    sheet.getRangeByName('G4').setText('สัญญาทั้งหมด');
/////////------------------------------------------->
    int index1 = 0;
    int indextotol = 0;
    List cid_number = [];
...
/////////---------------------------->
    ...
    String path = await FileSaver.instance.saveFile(
        (zone_name_Pe_Mon == null)
            ? 'รายงานพื้นที่เช่าที่มีสัญญาทับซ้อน(กรุณาเลือกโซน)'
            : 'รายงานพื้นที่เช่าที่มีสัญญาทับซ้อน(โซน : $zone_name_Pe_Mon)',
        data,
        "xlsx",
        mimeType: type);
    log(path);
