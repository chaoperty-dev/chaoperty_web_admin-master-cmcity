...
// (ทางที่ดี) log แบบย่อ ไม่ //print ค่าลับ
                                                                                        //   debug//print('POST /c_trans_installments -> typeInstall=$typeInstall, term=${params['term']}');
                            ...
                            Expanded(
                              child: Container(
                                ...
                                child: Text(
                                  nFormat.format(
                                    sum_amt -
                                        (double.tryParse(sum_disamt.text) ?? 0),
                                  ),
                                  textAlign: TextAlign.end,
                                  style: const TextStyle(
                                    color: PeopleChaoScreen_Color.Colors_Text2_,
                                ...
                                onChanged: (value) async {
                                  final i =
                                      TitleType_Default_Receipt_.indexWhere(
                                          (e) => e == value);
                                  setState(() => TitleType_Default_Receipt = i);
                                },
                              ),
                            ),
                          ],
                      ...
                      onPressed: _savingInvoice
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                // ใช้ tryParse กันพัง
                                final tableData003 = [
                                  for (int i = 0; i < _TransModels.length; i++)
                                    [
                                      '${i + 1}',
                                      '${_TransModels[i].date}',
                                      '${_TransModels[i].expname}',
                                      nFormat.format(double.tryParse(
                                              _TransModels[i].nvat ?? '0') ??
                                          0),
                                      nFormat.format(double.tryParse(
                                              _TransModels[i].vat ?? '0') ??
                                          0),
                                      nFormat.format(double.tryParse(
                                              _TransModels[i].pvat ?? '0') ??
                                          0),
                                      nFormat.format(double.tryParse(
                                              _TransModels[i].amt ?? '0') ??
                                          0),
                                    ],
                                ];
  ...
  Future<void> in_Trans_invoice2(
    ...
    try {
      ...
      if (code != 200) {
        ...
        try {
          ...
          final msg = (obj is Map && obj['error'] != null)
              ? obj['error'].toString()
              : raw;
          //debug//print('HTTP $code: $msg');
        } catch (_) {
          //debug//print('HTTP $code: $raw');
        }
        return;
      }

      if (raw == 'No') {
        // debug//print('❌ In_tran_invoice.php returned "No"');
        return;
      }
      ...
      try {
        ...
        //debug//print('❌ JSON decode failed: $raw');
        return;
      }
      ...
      if (result is List) {
        for (final map in result) {
          try {
            ...
            // debug//print('✅ docno: $docnoInv');
          } catch (e) {
            // debug//print('parse TransBillModel error: $e');
          }
        }
      }
      ...
      // debug//print('✅ วางบิลสำเร็จ');
    } on DioException catch (e) {
      final type = e.type;
      if (type == DioExceptionType.connectionTimeout ||
          type == DioExceptionType.receiveTimeout ||
          type == DioExceptionType.sendTimeout) {
        // debug//print('⏱️ Timeout: ${e.message}');
      } else {
        // debug//print('❌ Dio error: ${e.message}');
      }
    } catch (e) {
      //debug//print('❌ in_Trans_invoice2 error: $e');
    }
  }
...
//           allowSharing: true,
//           allow//printing: true, canDebug: false,
