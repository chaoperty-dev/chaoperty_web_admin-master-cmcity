...
// ExcelSheetProtectionOption
    ...
    sheet.getRangeByName('A4').setText('ทั้งหมด :${InvoiceModels.length}');
    sheet.getRangeByName('A5').setText('อัพเดต ณ :${DateTime.now()}');
    /////////---------->
    int columns_now =
        int.parse('${expModels.length}') * int.parse('${Type_vat.length}');
    /////////---------->
    for (int ix = 0; ix < 6; ix++) {
      sheet.getRangeByName('A${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('B${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('C${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('D${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('E${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('F${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('G${1 + ix}').cellStyle = globalStyle22;
      sheet.getRangeByName('H${1 + ix}').cellStyle = globalStyle22;
      /////////------------->
      int xx_count_1 = 0;
      for (int i = 0; i < expModels.length; i++) {
        for (int x = 0; x < Type_vat.length; x++) {
          ...
          sheet
              .getRangeByName('${columns[7 + xx_count_1]}${ix + 1}')
      ...
      //   }
      sheet
          .getRangeByName('${columns[7 + (columns_now + 1)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 2)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 3)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 4)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 5)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 6)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 7)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 8)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 9)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 10)]}${1 + ix}')
          .cellStyle = globalStyle22;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 11)]}${1 + ix}')
          .cellStyle = globalStyle22;
      // sheet
      //     .getRangeByName(
    ...
    for (int i = 0; i < expModels.length; i++) {
      for (int x = 0; x < Type_vat.length; x++) {
        ...
        sheet.getRangeByName('${columns[7 + xx_count_2]}6').cellStyle =
            globalStyle220;
        sheet.getRangeByName('${columns[7 + xx_count_2]}6').columnWidth = 25;
      }
    }
    // for (int i = 0; i < expModels.length * 2; i++) {
    ...
    //       globalStyle220;
    // }
    sheet.getRangeByName('${columns[7 + (columns_now + 1)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 2)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 3)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 4)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 5)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 6)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 7)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 8)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 9)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 10)]}6').cellStyle =
        globalStyle1;
    sheet.getRangeByName('${columns[7 + (columns_now + 11)]}6').cellStyle =
        globalStyle1;
    // sheet
    //     .getRangeByName(
    ...
    // }
    sheet.getRangeByName('${columns[7 + (columns_now + 1)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 2)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 3)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 4)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 5)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 6)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 7)]}6').columnWidth = 25;
    sheet.getRangeByName('${columns[7 + (columns_now + 8)]}6').columnWidth = 25;
    sheet.getRangeByName('${columns[7 + (columns_now + 9)]}6').columnWidth = 18;
    sheet.getRangeByName('${columns[7 + (columns_now + 10)]}6').columnWidth =
        18;
    sheet.getRangeByName('${columns[7 + (columns_now + 11)]}6').columnWidth =
        18;
    // sheet
    //     .getRangeByName(
    //         '${columns[7 + (int.parse('${expModels.length}') * 2 + 12)]}6')
    ...
    for (int i = 0; i < expModels.length; i++) {
      for (int x = 0; x < Type_vat.length; x++) {
        xx_count = xx_count + 1;
        sheet.getRangeByName('${columns[7 + xx_count]}6').setText(
    ...
    sheet.getRangeByName('${columns[7 + (columns_now + 1)]}6').setText(
          'ภาษีมูลค่าเพิ่ม',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 2)]}6').setText(
          'ภาษีหัก ณ ที่จ่าย',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 3)]}6').setText(
          'ส่วนลด',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 4)]}6').setText(
          'ยอดรวม',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 5)]}6').setText(
          'ยอดสุทธิ',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 6)]}6').setText(
          'หมายเหตุ',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 7)]}6').setText(
          'โซน',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 8)]}6').setText(
          'เลขที่-รับชำระ',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 9)]}6').setText(
          'ค่าปรับ-รับชำระ',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 10)]}6').setText(
          'ส่วนลด-รับชำระ',
        );
    sheet.getRangeByName('${columns[7 + (columns_now + 11)]}6').setText(
          'ยอดสุทธิ-รับชำระ',
        );
    // sheet
    //     .getRangeByName(
    ...
    for (int index = 0; index < InvoiceModels.length; index++) {
      ...
      for (int i = 0; i < expModels.length; i++) {
        for (int x = 0; x < Type_vat.length; x++) {
          ...
          sheet
              .getRangeByName('${columns[7 + xx_count_Color]}${index + 7}')
      ...
      //   }
      sheet
          .getRangeByName('${columns[7 + (columns_now + 1)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 2)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 3)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 4)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 5)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 6)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 7)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 8)]}${index + 7}')
          .cellStyle = (InvoiceModels[index].docno == null ||
              InvoiceModels[index].docno.toString() == '')
          ? numberColor2
          : numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 9)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 10)]}${index + 7}')
          .cellStyle = numberColor;
      sheet
          .getRangeByName('${columns[7 + (columns_now + 11)]}${index + 7}')
          .cellStyle = numberColor;
      // sheet
      //     .getRangeByName(
...
///////----------------------------->
      ...
      for (int index2 = 0; index2 < expModels.length; index2++) {
        ...
        double dis = (dataList.isEmpty)
            ...
            : dataList
                ...
                .where((element) =>
                    element['ser_exp'].toString() == '${expModels[index2].ser}')
                .map((element) => double.parse(element['dis_exp'].toString()))
                .fold(0, (prev, wht) => prev + wht);
    ...
    for (int i = 0; i < 5; i++) {
      sheet
          .getRangeByName(
              '${columns[7 + (columns_now + (i + 1))]}${InvoiceModels.length + 7}')
          .cellStyle = globalStyle88;
      ...
      sheet
          .getRangeByName(
              '${columns[7 + (columns_now + i + 1)]}${InvoiceModels.length + 7}')
          .setFormula(
              '=SUM(${columns[7 + (columns_now + i + 1)]}7:${columns[7 + (columns_now + i + 1)]}${indextotol + 7 - 1})');
    }
    for (int i = 0; i < 3; i++) {
      sheet
          .getRangeByName(
              '${columns[7 + (columns_now + 9 + i)]}${InvoiceModels.length + 7}')
          .cellStyle = globalStyle88;
      ...
      sheet
          .getRangeByName(
              '${columns[7 + (columns_now + 9 + i)]}${InvoiceModels.length + 7}')
          .setFormula(
              '=SUM(${columns[7 + (columns_now + 9 + i)]}7:${columns[7 + (columns_now + 9 + i)]}${indextotol + 7 - 1})');
    }
