...
import '../Model/trans_re_billCredit_Notehistory_model.dart';
import '../Model/trans_re_bill_history_model.dart';
import '../PDF/PDF_Receipt/pdf_AC_his_statusbill.dart';

import '../PDF_TP10/PDF_CreditNote_TP10/pdf_credit_note_TP10.dart';
...
import '../PDF_TP3/PDF_CreditNote_TP3/pdf_credit_note_TP3.dart';
import '../PDF_TP3/PDF_Receipt_TP3/pdf_AC_his_statusbill_TP3.dart';
import '../PDF_TP4/PDF_CreditNote_TP4/pdf_credit_note_TP4.dart';
...
import '../PDF_TP7/PDF_CreditNote_TP7/pdf_credit_note_TP7.dart';
import '../PDF_TP7/PDF_Receipt_TP7/pdf_AC_his_statusbill_TP7.dart';
import '../PDF_TP7_Ama1000/PDF_CreditNote_TP7/pdf_credit_note_TP7.dart';
import '../PDF_TP7_Ama1000/PDF_Receipt_TP7/pdf_AC_his_statusbill_TP7.dart';
import '../PDF_TP8/PDF_CreditNote_TP8/pdf_credit_note_TP8.dart';
import '../PDF_TP8/PDF_Receipt_TP8/pdf_AC_his_statusbill_TP8.dart';
import '../PDF_TP8_Choice/PDF_CreditNote_TP8/pdf_credit_note_TP8_Choice.dart';
import '../PDF_TP8_Choice/PDF_Receipt_TP8_Choice/pdf_AC_his_statusbill_TP8_Choice.dart';
import '../PDF_TP8_Ortorkor/PDF_CreditNote_TP8_Ortorkor/pdf_credit_note_TP8_Ortorkor.dart';
import '../PDF_TP9/PDF_CreditNote_TP9/pdf_credit_note_TP9.dart';
import '../PDF_TP9/PDF_Receipt_TP9/pdf_AC_his_statusbill_TP9.dart';
import '../PDF_TP9_Lao/PDF_CreditNote_TP9/pdf_credit_note_TP9.dart';
...
class ManCredit_Note_PDF {
  // --------------------------------> PDF ใบลดหนี้ หรือ Credit Note

  static void Man_creditnote_PDF(
      docno, ////----->เลขที่รับชำระ
      docno_inv,
      context, ////----->context
      foder, ////----->foder
      ...
      bills_name,
      Preview_ser) async {
    print('GC_Data_OnBill_PDF>>>> Credit_Note_PDF');
    List<FinnancetransModel> finnancetransModels = [];
    List<TransReBillCredit_NoteHistoryModel> _TransReBillHistoryModels = [];
    ...
    double sum_addvat = 0.00, sum_Nonvat = 0.00, total_bill_befor = 0.00;
    String numinvoice = '';
    String numdoctax = '';
...
//////////////////////-------------------------------------------->
    ...
    try {
      ...
      if (result.toString() != 'null') {
        ...
        for (var map in result) {
          TransReBillCredit_NoteHistoryModel _TransReBillHistoryModel =
              TransReBillCredit_NoteHistoryModel.fromJson(map);
          numinvoice = _TransReBillHistoryModel.docno!;
          numdoctax = _TransReBillHistoryModel.doctax!;
          var sum_pvatx = (_TransReBillHistoryModel.pvat_cn == null)
              ? 0.00
              : double.parse(_TransReBillHistoryModel.pvat_cn!);
          var sum_amtx = (_TransReBillHistoryModel.amt_cn == null)
              ? 0.00
              : double.parse(_TransReBillHistoryModel.amt_cn!);
          var sum_vatx = (_TransReBillHistoryModel.vat_cn == null)
              ? 0.00
              : double.parse(_TransReBillHistoryModel.vat_cn!);
          var sum_whtx = (_TransReBillHistoryModel.wht_cn == null)
              ? 0.00
              : double.parse(_TransReBillHistoryModel.wht_cn!);
          var total_bill_beforx = (_TransReBillHistoryModel.total_bill == null)
              ? 0.00
              : double.parse(_TransReBillHistoryModel.total_bill!);
          var sum_net_amount_pvat =
              (_TransReBillHistoryModel.total_bill == null)
                  ? 0.00
                  : double.parse(_TransReBillHistoryModel.net_amount_pvat!);
          var sum_net_non_pvat = (_TransReBillHistoryModel.net_non_pvat == null)
              ? 0.00
          ...
          total_bill_befor = total_bill_beforx;
...
/////////////////------------------------------------------>
        ...
        ? [
            for (int index = 0;
              ...
              [
                ...
                ///---19
                '${nFormat.format((_TransReBillHistoryModels[index].amt_cn == null) ? 0.00 : double.parse(_TransReBillHistoryModels[index].amt_cn!))}',

                ///---20
                '${nFormat.format((_TransReBillHistoryModels[index].vat_cn == null) ? 0.00 : double.parse(_TransReBillHistoryModels[index].vat_cn!))}',

                ///---21

                '${nFormat.format((_TransReBillHistoryModels[index].pvat_cn == null) ? 0.00 : double.parse(_TransReBillHistoryModels[index].pvat_cn!))}',

                ///---22
                '${nFormat.format((_TransReBillHistoryModels[index].wht_cn == null) ? 0.00 : double.parse(_TransReBillHistoryModels[index].wht_cn!))}',

                ///---23
                '${nFormat.format((_TransReBillHistoryModels[index].pri_cn == null) ? 0.00 : double.parse(_TransReBillHistoryModels[index].pri_cn!))}',
    ...
    if (sum_fine != 0 && count_inv == 0) {
      tableData01.add(
        [
          ...
          ///---11
          '${nFormat.format(_TransReBillHistoryModels.where((model) => model.fine == '1' || model.fine == '1.00').map((model) => double.parse(model.amt_cn ?? '0.00')).fold(0.0, (previousValue, element) => previousValue + element))}',

          ///---12
          '${nFormat.format(_TransReBillHistoryModels.where((model) => model.fine == '1' || model.fine == '1.00').map((model) => double.parse(model.vat_cn ?? '0.00')).fold(0.0, (previousValue, element) => previousValue + element))}',

          ///---13
          '${nFormat.format(_TransReBillHistoryModels.where((model) => model.fine == '1' || model.fine == '1.00').map((model) => double.parse(model.pvat_cn ?? '0.00')).fold(0.0, (previousValue, element) => previousValue + element))}',

          ///---14
          '${nFormat.format(_TransReBillHistoryModels.where((model) => model.fine == '1' || model.fine == '1.00').map((model) => double.parse(model.wht_cn ?? '0.00')).fold(0.0, (previousValue, element) => previousValue + element))}',

          ///---15
          '${nFormat.format(_TransReBillHistoryModels.where((model) => model.fine == '1' || model.fine == '1.00').map((model) => double.parse(model.pri_cn ?? '0.00')).fold(0.0, (previousValue, element) => previousValue + element))}',
    ...
    Future.delayed(Duration(milliseconds: 500), () async {
      if (tem_page_ser.toString() == '0' || tem_page_ser == null) {
        Pdfgen_credit_note_TP3.exportPDF_creditnote_TP3(
            Cust_no,
            cid_,
            Zone_s,
            Ln_s,
        ...
        Pdfgen_credit_note_TP4.exportPDF_creditnote_TP4(
            Cust_no,
            cid_,
            Zone_s,
            Ln_s,
        ...
        if (rtser.toString() == '102') {
          Pdfgen_credit_note_TP7_Ama.exportPDF_creditnote_TP7_Ama(
              Cust_no,
              cid_,
              Zone_s,
              Ln_s,
          ...
          Pdfgen_credit_note_TP7.exportPDF_creditnote_TP7(
              Cust_no,
              cid_,
              Zone_s,
              Ln_s,
        ...
        if (rtser.toString() == '72' ||
            ...
            rtser.toString() == '93' ||
            rtser.toString() == '94') {
          Pdfgen_credit_note_TP8_Ortorkor.exportPDF_creditnote_TP8_Ortorkor(
              Cust_no,
              cid_,
              Zone_s,
              Ln_s,
          ...
          Pdfgen_credit_note_TP8_Choice.exportPDF_creditnote_TP8_Choice(
              Cust_no,
              cid_,
              Zone_s,
              Ln_s,
          ...
          Pdfgen_credit_note_TP8.exportPDF_creditnote_TP8(
              Cust_no,
              cid_,
              Zone_s,
              Ln_s,
        ...
        Pdfgen_credit_notel_TP9_Lao.exportPDF_creditnote_TP9_Lao(
            Cust_no,
            cid_,
            Zone_s,
            Ln_s,
        ...
        ///แคนนาสสส
        Pdfgen_credit_note_TP9.exportPDF_creditnote_TP9(
            Cust_no,
            cid_,
            Zone_s,
            Ln_s,
        ...
        Pdfgen_credit_note_TP10.exportPDF_creditnote_TP10(
            Cust_no,
            cid_,
            Zone_s,
            Ln_s,
