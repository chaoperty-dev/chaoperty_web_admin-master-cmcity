...
import '../PDF/Choice/Cancell_Agreement_Choice/pdf_Cancell_Agreement_Choice.dart';
...
///////////----------------------------------------------->
    ...
    try {
      ...
      var result = json.decode(response.body);
      print('result GC_quot_conx : $result');
...
///////////-----------------------------------------------> GC_tenantCancel_PDF
    ...
    try {
      var response = await httpClient.get(Uri.parse(url3));

      var result = json.decode(response.body);
      print('result tran kon : $result');
      if (result.toString() != 'null') {
        for (var map in result) {
...
///////////----------------------------------------------->
    ...
    try {
      var response = await httpClient.get(Uri.parse(url4));

      var result = json.decode(response.body);
      print('result GC_tenantCancel_PDF : $result');
      if (result.toString() != 'null') {
        for (var map in result) {
          TeNantModel teNantModelsCancel = TeNantModel.fromJson(map);
          Formbecause_cancel = teNantModelsCancel.cc_remark?.toString() ?? '';
          var rawCcDate = teNantModelsCancel.cc_date?.toString();
          cc_date =
              (rawCcDate != null && rawCcDate != 'null' && rawCcDate.isNotEmpty)
                  ? rawCcDate
                  : null;
          var rawW1Date = teNantModelsCancel.w1?.toString();
          w1_date =
              (rawW1Date != null && rawW1Date != 'null' && rawW1Date.isNotEmpty)
                  ? rawW1Date
                  : null;
        }
      }
    ...
    String url5 =
        '${MyConstant().domain}/GC_PakanDocno_Choice.php?isAdd=true&ren=$ren&ciddoc=$ciddoc&renew_cid=$ciddoc';

    try {
      var response5 = await httpClient.get(Uri.parse(url5));

      var result5 = json.decode(response5.body);
      print(url5);
      if (result5.toString() != 'null') {
        for (var map in result5) {
          PakanDocnoModel pakanDocnoModel = PakanDocnoModel.fromJson(map);
          Form_expnamePakan = pakanDocnoModel.expname_pakan ?? '-';
          Form_expserPakan = pakanDocnoModel.expser_pakan ?? '0';

          Form_PakanAll_pvat = (pakanDocnoModel.pvat_pakan == null ||
                  pakanDocnoModel.pvat_pakan.toString() == '')
              ? '0.00'
              : pakanDocnoModel.pvat_pakan.toString();
          Form_PakanAll_vat = (pakanDocnoModel.vat_pakan == null ||
                  pakanDocnoModel.vat_pakan.toString() == '')
              ? '0.00'
              : pakanDocnoModel.vat_pakan.toString();

          Form_PakanAll_Total = (pakanDocnoModel.total_pakan == null ||
    ...
    String url6 =
        '${MyConstant().domain}/GC_Type_paper.php?isAdd=true&ren=$ren';

    try {
      var response6 = await httpClient.get(Uri.parse(url6));

      var result6 = json.decode(response6.body);
      // print('read_GC_rental///// $result');
      for (var map in result6) {
        TypePaperModel typePaperModelss = TypePaperModel.fromJson(map);

        typePaperModels.add(typePaperModelss);
      }
      cid_typePaper =
          await '${typePaperModels.where((model) => model.ser.toString() == '${(cid_typePaper_ser.toString() == '0') ? 1 : cid_typePaper_ser}').map((model) => model.p_type).join(',')}';
    } catch (e) {}
///////////----------------------------------------------->
    Future.delayed(Duration(milliseconds: 500), () async {
      print('ciddoc: $ciddoc <====================>qutser : $qutser');
      print('**TeNant_typeshop : $TeNant_typeshop');
      print('**Form_expserPakan : $Form_expserPakan');
      print('**Form_expnamePakan : $Form_expnamePakan');
      print('**Form_PakanAll_pvat : $Form_PakanAll_pvat');
      print('**cid_typePaper : $cid_typePaper');
      print('ciddoc: $ciddoc <====================>qutser : $qutser');
      (rt_Language.toString().trim() == 'LA')
          ...
          : (ren.toString().trim() == '106')
              ? Pdfgen_Cancell_Agreement_Choice
                  .exportPDF_Cancell_Agreement_Choice(
                      context,
                      foder,
                      renTal_name,
                      bill_addr,
                      ...
                      qutser,
                      TeNant_verticalGroupValue,
                      // TeNant_sname, // TODO: User local change - variable undefined
                      // TeNant_room_number, // TODO: User local change - variable undefined
                      null,
                      null,
                      newValuePDFimg,
                      Formbecause_cancel,
