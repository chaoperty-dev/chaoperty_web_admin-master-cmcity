...
class Excgen_BillPayMonRentReport_Choice {
  static void exportExcel_BillPayMonRentReport_Choice(
    ...
    sheet.name = 'รายงานค่าเช่าประจำเดือนฉบับเต็ม';
...
////////////-------------------------------------------------------->
    ...
    sheet
        .getRangeByName('A2')
        .setText('เดือนที่รับชำระ ${Mon_billpay_Mon} ${YE_billpay_Mon}');
    ...
    sheet.getRangeByName('A5').setText('ทั้งหมด :${billpay_Mon.length}');
    sheet.getRangeByName('J5').setText('อัพเดต ณ :${DateTime.now()}');
...
// Protecting the Worksheet by using a Password
    ...
    // sheet.getRangeByName('AA6').cellStyle = globalStyle2220;
    ...
    sheet.getRangeByName('E6').columnWidth = 15;
    ...
    sheet.getRangeByName('R6').columnWidth = 35;
    ...
    sheet.getRangeByName('I6').setText('ค่าเช่าพื้นที่ดิน');

    sheet.getRangeByName('J6').setText('ค่าเช่าพื้นที่');
    sheet.getRangeByName('K6').setText('ค่าบริการพื้นที่');
    sheet.getRangeByName('L6').setText('ค่าน้ำ');
    sheet.getRangeByName('M6').setText('ค่าไฟ');
    sheet.getRangeByName('N6').setText('ค่าธรรมเนียม-ค่าน้ำ');
    sheet.getRangeByName('O6').setText('ค่าธรรมเนียม-ค่าไฟ');

    sheet.getRangeByName('P6').setText('เดือน');
    sheet.getRangeByName('Q6').setText('วันที่เงินเข้าบัญชี');
    sheet.getRangeByName('R6').setText('ส่วนลด');
    sheet.getRangeByName('S6').setText('ค่าปรับ');
    sheet.getRangeByName('T6').setText(
        'ยอดรวมทั้งหมด'); //ค่าเช่าพื้นที่ดิน+ค่าเช่า+น้ำ+ไฟฟ้า+vat+ค่าปรับ(ในระบบ)

    sheet.getRangeByName('U6').setText('เงินเข้าบัญชี');

    sheet.getRangeByName('V6').setText('ผลต่าง');
    sheet.getRangeByName('W6').setText('สถานะ');

    sheet.getRangeByName('X6').setText('ผู้ดูแล');
    sheet.getRangeByName('Y6').setText('หมายเหตุ');

    sheet.getRangeByName('Z6').setText('ref1');
    sheet.getRangeByName('AA6').setText('ref2');
    sheet.getRangeByName('AB6').setText('ref-chao');
    // sheet.getRangeByName('AA6').setText('ref4');
    ...
    for (int index = 0; index < billpay_Mon.length; index++) {
      ...
      sheet.getRangeByName('C${index + 7}').setText(
          (billpay_Mon[index].zn == null) ? '' : '${billpay_Mon[index].zn}');
      sheet.getRangeByName('D${index + 7}').setText(
          (billpay_Mon[index].ln == null) ? '' : '${billpay_Mon[index].ln}');
      ...
      sheet.getRangeByName('I${index + 7}').setNumber(
          (billpay_Mon[index].land_total == null)
              ? 0.00
              : double.parse('${billpay_Mon[index].land_total}'));
      ...
      sheet.getRangeByName('N${index + 7}').setNumber(
          (billpay_Mon[index].water_fees == null)
              ? 0.00
              : double.parse('${billpay_Mon[index].water_fees}'));
      sheet.getRangeByName('O${index + 7}').setNumber(
          (billpay_Mon[index].electricity_fees == null)
              ? 0.00
              : double.parse('${billpay_Mon[index].electricity_fees}'));

      sheet.getRangeByName('P${index + 7}').setValue(
            (billpay_Mon[index].daterec == null ||
                    billpay_Mon[index].daterec.toString() == '')
                ? null
                : DateTime.parse('${billpay_Mon[index].daterec}'),
          );
      ...
      // sheet.getRangeByName('P${index + 7}').setText((billpay_Mon[index]
      //             .daterec ==
      //         null)
      //     ? ''
      ...
      sheet.getRangeByName('U${index + 7}').setNumber(
          (billpay_Mon[index].total_bill_pay == null)
              ? 0.00
              : double.parse('${billpay_Mon[index].total_bill_pay}'));
      sheet
          .getRangeByName('V${index + 7}')
          .setFormula('=U${index + 7}-T${index + 7}');
      // sheet.getRangeByName('S${index + 7}').setNumber(double.parse(
      //         (billpay_Mon[index].total_bill == null)
      //             ? '0.00'
      //             : '${billpay_Mon[index].total_bill}') -
      //     double.parse((billpay_Mon[index].total_bill_pay == null)
      //         ? '0.00'
      //         : '${billpay_Mon[index].total_bill_pay}'));
      sheet.getRangeByName('W${index + 7}').setText(
          (billpay_Mon[index].st == null) ? '' : '${billpay_Mon[index].st}');
      sheet.getRangeByName('X${index + 7}').setText(
          (billpay_Mon[index].name_user == null)
              ? ''
              : '${billpay_Mon[index].name_user}');
      sheet.getRangeByName('Y${index + 7}').setText(
          (billpay_Mon[index].remark == null)
              ? ''
      ...
      sheet.getRangeByName('AA${index + 7}').setText(
          (billpay_Mon[index].ref4 == null)
              ? ''
              : '${billpay_Mon[index].ref4}');

      sheet.getRangeByName('AB${index + 7}').setText(
      ...
      //     .setText('${billpay_Mon[index].ref4}');

      indextotol = indextotol + 1;
    }
/////////---------------------------->
    sheet.getRangeByName('G${indextotol + 7 + 0}').setText('รวมทั้งหมด: ');
    sheet
    ...
    sheet
        ...
        .setFormula('=SUM(R7:R${indextotol + 3 - 1})');
    sheet
...
/////////---------------------------->
    ...
    String path = await FileSaver.instance.saveFile(
        (Value_Chang_Zone_billpayMon == null)
            ? 'รายงานค่าเช่าฉบับเต็ม(กรุณาเลือกโซน)'
            : 'รายงานค่าเช่าฉบับเต็ม(โซน : $Value_Chang_Zone_billpayMon)',
        data,
        "xlsx",
        mimeType: type);
    log(path);
