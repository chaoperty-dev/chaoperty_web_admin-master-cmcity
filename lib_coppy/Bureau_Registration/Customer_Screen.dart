...
import 'update_custo_exc_screen.dart';
...
///////////////------------------------------------------------------->
  Future<void> Save_FormText() async {
    ...
    if (ren == null || ren.isEmpty) {
      // แจ้งเตือนตาม UI ของคุณ
      //  print('renTalSer missing');
      return;
    }

    // ค่าจากฟอร์ม (trim เพื่อลดปัญหาช่องว่าง)
    final ser = _Form_Ser.toString().trim();
    final nameshop = Form_nameshop_.text.trim();
    final typeshop = Form_typeshop_.text.trim();
    final bussshop = Form_bussshop_.text.trim();
    final isPersonal =
        _verticalGroupValue.toString().trim() == 'ส่วนตัว/บุคคลธรรมดา';
    final bussscontact = isPersonal ? bussshop : Form_bussscontact_.text.trim();
    final address = Form_address_.text.trim();
    final tel = Form_tel_.text.trim();
    final email = Form_email_.text.trim();
    final tax = Form_tax_.text.trim();
    final typerser =
        (int.tryParse(Value_AreaSer_.toString()) ?? 0) + 1; // 1/2/3...
    final birth = Status4Form_birth.text.trim();
    final religion = Status4Form_religion.text.trim();
    final national = Status4Form_national.text.trim();

    // ใช้ POST form-urlencoded (เสถียรกว่า GET มาก)
    ...
    // รบกวนตรวจกับฝั่ง PHP ว่ารับ key ตามนี้นะ
    final body = <String, String>{
      'user': ser, // ถ้า PHP ใช้ชื่อ 'user' เป็น ser ของลูกค้า
      'nameshop': nameshop,
      'typeshop': typeshop,
      'bussshop': bussshop,
      ...
      'tax': tax,
      'type':
          _verticalGroupValue.toString(), // เก็บคำว่า "ส่วนตัว/บุคคลธรรมดา" ได้
      'typeser':
          typerser.toString(), // 1 = บุคคลธรรมดา, 2 = นิติ ฯลฯ (ตามระบบคุณ)
      'sertap': Ser_Tap.toString(), // แท็บหน้า UI ปัจจุบัน
      'birth': birth,
      'religion': religion,
      'national': national,
      // ถ้าฝั่ง PHP ต้องการ ser ของผู้ใช้ที่ทำรายการ
      'ser_user': serUser ?? '',
    };
    ...
    try {
      ...
      // บาง backend คืน "true" (string) บางที่คืน JSON
      ...
      try {
        ...
        if (decoded is bool) {
          ...
          ok = decoded.toLowerCase() == 'true';
        } else if (decoded is Map) {
          // รองรับ {"ok":true} หรือ {"success":true}
          ok = (decoded['ok'] == true) || (decoded['success'] == true);
        } else if (decoded is List && decoded.isNotEmpty) {
          // บางที่อาจคืนเป็น list ผลลัพธ์ แปลว่าทำสำเร็จ
          ok = true;
        }
      } catch (_) {
        // ไม่ใช่ JSON ก็อาจเป็น "true" ธรรมดา
        ok = raw.toLowerCase() == 'true' ||
      ...
      if (ok) {
        ...
        if (Ser_Tap == 0) {
          ...
          Insert_log.Insert_logs(
              'ทะเบียน', 'ทะเบียนลูกค้าประเภทล็อกเสียบ>>แก้ไข($nameshop/$tax)');
        }

        // รีโหลดรายการ
        await select_coutumer();
        Dialog_success(context, 'แก้ไขสำเร็จ');
      } else {
        //  print('บันทึกไม่สำเร็จ: $raw');
      }
    } catch (e) {
      // print('Save_FormText error: $e');
    }
  }
  ...
  //   String? birth = Status4Form_birth.text.toString();
  //   String? religion = Status4Form_religion.text.toString();
  //   String? national = Status4Form_national.text.toString();

  //   String url =
  //       '${MyConstant().domain}/Inc_customer_Bureau.php?isAdd=true&ren=$ren&user=$Ser&nameshop=$nameshop &typeshop=$typeshop&bussshop=$bussshop&bussscontact=$bussscontact&address=$address&tel=$tel &email=$email&tax=$tax&type=$_verticalGroupValue&typeser=$typerser&sertap=$Ser_Tap  &birth = $birth &religion = $religion& national = $national';
                                                                        ...
                                                                        return Material(
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            child:
                                                                                ListTile(
                                                                              onTap: () async {
                                                                                String custno_ = customerModels[index].custno == null ? '' : '${customerModels[index].custno}';
                                                                                setState(() {
                                                                                  in_dex = index;
                                                                                  cust_no_ = custno_;
                                                                                  Total_amtbill = 0;
                                                                                ...
                                                                                setState(() {
                                                                                  ...
                                                                                  tappedIndex_2 = '';
                                                                                  _Form_Ser = (Ser_Tap == 0) ? '${customerModels[index].ser}' : '${customerModels[index].custno}';

                                                                                  _verticalGroupValue = '${customerModels[index].type}'; // ประเภท

                                                                                  Form_nameshop_.text = (customerModels[index].scname.toString() == 'null') ? '' : '${customerModels[index].scname}';
                                                                                  // Form_User
                                                                                  //     .text = customerModels[
                                                                                  //         index]
                                                                                  //     .user_name!;
                                                                                  Form_typeshop_.text = '${customerModels[index].stype}';
                                                                                  Form_Img_ = '${customerModels[index].addr2}';
                                                                                  Form_bussshop_.text = '${customerModels[index].cname}';
                                                                                  Form_bussscontact_.text = '${customerModels[index].attn}';
                                                                                  Form_address_.text = '${customerModels[index].addr1}';
                                                                                  Form_tel_.text = '${customerModels[index].tel}';
                                                                                  Form_email_.text = '${customerModels[index].email}';
                                                                                  Form_tax_.text = customerModels[index].tax == 'null' ? "-" : '${customerModels[index].tax}';

                                                                                  Status4Form_birth.text = customerModels[index].birth ?? "";
                                                                                  Status4Form_religion.text = customerModels[index].religion ?? "";
                                                                                  Status4Form_national.text = customerModels[index].national ?? '';
                                                                                });
                                                                                ...
                                                                                for (int i = 0; i < typeModels.length; i++) {
                                                                                  ...
                                                                                  // print(
                                                                                  //     '--------------------------------------');
                                                                                  if (customerModels[index].type.toString() == typeModels[i].type.toString()) {
                                                                                    setState(() {
                                                                                      Value_AreaSer_ = int.parse(typeModels[i].ser.toString()) - 1;
                                                                                    });
                                                                                  } else {}
                                                                                }
                                                                                  ...
                                                                                  children: [
                                                                                    ...
                                                                                    Expanded(
                                                                                      ...
                                                                                      child: AutoSizeText(
                                                                                        minFontSize: 10,
                                                                                        maxFontSize: 18,
                                                                                        (Ser_Tap == 0) ? '${customerModels[index].tax}' : '${customerModels[index].tax}',
                                                                                        textAlign: TextAlign.left,
                                                                                        style: const TextStyle(
                                                                                            color: CustomerScreen_Color.Colors_Text2_,
                                                                                            // fontWeight: FontWeight.bold,
                                                                ...
                                                                children: [
                                                                  if (Ser_Tap !=
                                                                    ...
                                                                    Padding(
                                                                      ...
                                                                      child:
                                                                          GestureDetector(
                                                                        onTap: (_TransReBillModels.length >
                                                                                0)
                                                                            ? null
                                                                            : (tappedIndex_ == '')
                                                                                ? null
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          decoration:
                                                                              BoxDecoration(
                                                                            color: (_TransReBillModels.length > 0 || tappedIndex_ == '')
                                                                                ? Colors.grey[300]
                                                                                : (tappedIndex_ == '')
                                                                                    ? Colors.grey[300]
                                                                                    : Colors.red[300],
                                                                      ...
                                                                      onTap:
                                                                        ...
                                                                        showDialog(
                                                                          ...
                                                                          barrierDismissible:
                                                                              false,
                                                                          builder: (_) =>
                                                                              UpdateCustoExcScreen(
                                                                            customerList:
                                                                                limitedList_customerModels,
                                                                          ),
                                                                        );
                                                                      },
                                                                      child:
                                                                        ...
                                                                        Expanded(
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              child: Translate.TranslateAndSetText('$_verticalGroupValue', CustomerScreen_Color.Colors_Text2_, TextAlign.start, null, Font_.Fonts_T, 14, 1),
                                                                            ),
                                                                          ),
                                                                        ),
                                                                        Expanded(
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            children: [
                                                                              Translate.TranslateAndSet_TextAutoSize('รูป(คลิกที่รูปเพื่อดู)', CustomerScreen_Color.Colors_Text1_, TextAlign.center, FontWeight.bold, FontWeight_.Fonts_T, 8, 20, 1),
                                                                              if (tappedIndex_ != '' && Ser_Tap != 1)
                                                                                IconButton(
                                                                                  onPressed: () async {
                                                                                    deleteFile();
                                                                                    setState(() {
                                                                      ...
                                                                      children: [
                                                                        ...
                                                                        Expanded(
                                                                          flex:
                                                                              1,
                                                                          child: Translate.TranslateAndSet_TextAutoSize(
                                                                              (Ser_Tap == 0) ? 'เช่าครั้งแรก(จนถึงปัจจุบัน)' : 'Password',
                                                                              CustomerScreen_Color.Colors_Text2_,
                                                                              TextAlign.center,
                                                                              FontWeight.bold,
                                                                              FontWeight_.Fonts_T,
                                                                        ...
                                                                        (Ser_Tap ==
                                                                            ...
                                                                            ? Expanded(
                                                                                ...
                                                                                child: Container(
                                                                                  ...
                                                                                  padding: const EdgeInsets.all(8.0),
                                                                                  child: (Form_MinSdate_.text == null || Form_MinSdate_.text == '') ? Text(' - ') : Translate.TranslateAndSet_TextAutoSize('${Form_MinSdate_.text} [ ${Calculate_sdate()} ] ', CustomerScreen_Color.Colors_Text2_, TextAlign.center, null, Font_.Fonts_T, 8, 20, 1),
                                                                                ),
                                                                              )
                                                                            : Expanded(
                                                                                flex: 2,
                                                                                    ...
                                                                                    child: Padding(
                                                                                      ...
                                                                                      child: Text(
                                                                                        ...
                                                                                        (Status4Form_birth.text.isNotEmpty)
                                                                                            ? (DateTime.tryParse(Status4Form_birth.text.trim())!.year <= 1900)
                                                                                                ? ''
                                                                                                : '${DateFormat('dd-MM-yyyy').format(DateTime.tryParse(Status4Form_birth.text.trim())!)}'
                                                                                            : '',
                                                                                        textAlign: TextAlign.center,
                                                                                        style: TextStyle(
                                                                                          color: Colors.black,
                                                                        ...
                                                                        children: [
                                                                          Expanded(
                                                                            ...
                                                                            child:
                                                                                Row(
                                                                              children: [
                                                                                Copy_Text(context, '${_TransReBillModels[index].docno}'),
                                                                                Expanded(
                                                                                  child: AutoSizeText(
                                                                                    minFontSize: 10,
                                                                                    maxFontSize: 18,
                                                                                    (_TransReBillModels[index].docno == null) ? '' : '${_TransReBillModels[index].docno}',
                                                                                    textAlign: TextAlign.center,
                                                                                    style: const TextStyle(
                                                                                        color: CustomerScreen_Color.Colors_Text2_,
                                                                                        // fontWeight: FontWeight.bold,
                                                                          ...
                                                                          Expanded(
                                                                            ...
                                                                            child:
                                                                                Row(
                                                                              children: [
                                                                                (_TransReBillModels[index].doctax == null || _TransReBillModels[index].doctax.toString() == '') ? SizedBox() : Copy_Text(context, '${_TransReBillModels[index].doctax}'),
                                                                                Expanded(
                                                                                  child: AutoSizeText(
                                                                                    minFontSize: 10,
                                                                                    maxFontSize: 18,
                                                                                    (_TransReBillModels[index].doctax == null) ? '' : '${_TransReBillModels[index].doctax}',
                                                                                    textAlign: TextAlign.center,
                                                                                    style: const TextStyle(
                                                                                        color: CustomerScreen_Color.Colors_Text2_,
                                                                                        // fontWeight: FontWeight.bold,
                                                                          ...
                                                                          Expanded(
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              minFontSize: 10,
                                                                              maxFontSize: 18,
                                                                              (_TransReBillModels[index].daterec == null) ? '' : '${_TransReBillModels[index].daterec}',
                                                                              textAlign: TextAlign.center,
                                                                              style: const TextStyle(
                                                                                  color: CustomerScreen_Color.Colors_Text2_,
                                                                                  // fontWeight: FontWeight.bold,
                                                                          ...
                                                                          Expanded(
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              minFontSize: 10,
                                                                              maxFontSize: 18,
                                                                              (_TransReBillModels[index].pdate == null) ? '' : '${_TransReBillModels[index].pdate}',
                                                                              textAlign: TextAlign.center,
                                                                              style: const TextStyle(
                                                                                  color: CustomerScreen_Color.Colors_Text2_,
                                                                                  // fontWeight: FontWeight.bold,
                                                                          ...
                                                                          Expanded(
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              child: AutoSizeText(
                                                                                minFontSize: 10,
                                                                                maxFontSize: 18,
                                                                                (_TransReBillModels[index].total_bill == null) ? '0.00' : '${nFormat.format(double.parse(_TransReBillModels[index].total_bill.toString()))}',
                                                                          ...
                                                                          Expanded(
                                                                              flex: 1,
                                                                              child: Translate.TranslateAndSet_TextAutoSize((_TransReBillModels[index].doctax == '') ? '-' : 'ใบกำกับภาษี', CustomerScreen_Color.Colors_Text2_, TextAlign.center, FontWeight.bold, FontWeight_.Fonts_T, 8, 20, 1)),
                                                                          // Expanded(
                                                                          //   flex: 1,
                                                                          //   child: Padding(
                                                                          //     padding:
