# Comment out (wrap in // /* ... // */) all listed unused dart files under lib/
# Preserves UTF-8 (no BOM) for Thai/Lao text content.
...
'lib/Responsive/tesi_print_name.dart'
...
foreach ($rel in $paths) {
    ...
    try {
        ...
        if ($content.StartsWith('// /*')) {
            Write-Host "SKIP (already wrapped): $rel"
            $skipped++
            continue
        }
        $wrapped = "// /*`n" + $content
        if (-not $content.EndsWith("`n")) { $wrapped += "`n" }
        $wrapped += "// */`n"
...
Write-Host "Skipped (already wrapped): $skipped"
